...
Private m_YPNSSTab          As ClsYPNSSTab

Public Sub MSSTabInit(ByVal i_SSTab As Object, Optional ByVal i_Style As Integer = 0)
    If Not (TypeOf i_SSTab Is SSTab) Then Err.Raise 5
    
    Call loadResDC(i_SSTab)
    
    Set m_YPNSSTab = New ClsYPNSSTab
    
    If m_YPNSSTab.IsAttached = False Then m_YPNSSTab.Attach i_SSTab
    ...
    ElseIf (1 = i_Style) Then
        m_YPNSSTab.UpDownWidth = 16
        m_YPNSSTab.UpDownHeight = 16
        
        m_YPNSSTab.TabWidth = 100
        m_YPNSSTab.TabHeight = 24
    ElseIf (2 = i_Style) Then
        m_YPNSSTab.UpDownWidth = 16
        m_YPNSSTab.UpDownHeight = 16
...
Private Sub DrawBackGround(ByVal hdc As Long, ByVal nWidth As Long, ByVal nHeight As Long)
    Dim hBrush As Long, hOldBrush  As Long
        
    Dim rcBackground As RECT
    Call SetRect(rcBackground, 0, 0, nWidth, nHeight)
    
    If (OptionTheme(1).Value = True) Then
        ' XP样式
        ...
        Call Rectangle(hdc, 0, m_customSSTab.TabHeight - 1, nWidth, nHeight)
    ...
    ElseIf (OptionTheme(2).Value = True) Then
        ' QQ样式
        hBrush = CreateSolidBrush(&HF9FCFB)
        Call FillRect(hdc, rcBackground, hBrush)
        Call GridBlt(hdc, 0, 0, nWidth, m_customSSTab.TabHeight, m_hQQResDC, 0, 0, 1, 24, 0, 0, 0, 1)
        Call DeleteObject(hBrush)
    ElseIf (OptionTheme(3).Value = True) Then
        ' Office样式
        hBrush = CreateSolidBrush(&HF6EAE1)
        Call FillRect(hdc, rcBackground, hBrush)
        Call GridBlt(hdc, 0, 0, nWidth, m_customSSTab.TabHeight, m_hOfficeResDC, 0, 0, 1, 27, 0, 0, 0, 1)
        Call DeleteObject(hBrush)
    End If
End Sub

Private Sub DrawTab(ByVal nTab As Long, ByVal nState As Long, ByVal hdc As Long, ByVal nleft As Long, ByVal nTop As Long, ByVal nWidth As Long, ByVal nHeight As Long)
    ...
    If (SSTab1.TabEnabled(nTab) = False) Then Call SetTextColor(hdc, RGB(150, 150, 150))
    
    Call DrawText(hdc, SSTab1.TabCaption(nTab), -1, rcTab, DT_CENTER Or DT_VCENTER Or DT_END_ELLIPSIS_CON Or DT_SINGLELINE)
    Call SetBkMode(hdc, nOldBkMode)
    Call SelectObject(hdc, hOldFont)
    Call SetTextColor(hdc, dwOldTextColor)
End Sub

Private Sub DrawUpDown(ByVal bUpButton As Boolean, ByVal nState As Long, ByVal hdc As Long, ByVal nleft As Long, ByVal nTop As Long, ByVal nWidth As Long, ByVal nHeight As Long)
